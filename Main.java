...
public class Main {
    ...
    public static void main(String[] a) throws Exception {
        System.out.println("ADMINISTRADOR DE CALIFICACIONES 3A (feo a proposito)");
        System.out.println("Materias validas: MAT, ESP, ING");
        seed();
        menu();
    }
...
class Estudiante3A {
    ...
    public int[] CAL = new int[]{0, 0, 0};
    public String[] MATs = new String[]{"MAT", "ESP", "ING"}; // public, nombres feos, etc

    public Estudiante3A(int i, String n, String g) {
        ID = i;
        NOMBRE = n;
        GRUPO = g;
        if (NOMBRE == null) {
...
class GestorCALIF {
    ...
    public String gpo;
    private ArrayList<Estudiante3A> L = new ArrayList<Estudiante3A>();
    public int next = 1;
    ...
    public Estudiante3A add(String nombre) {
        ...
        Estudiante3A e = new Estudiante3A(next, nombre, gpo);
        L.add(e);
        next = next + 1;
        return e;
    }

    public Estudiante3A getById(int id) {
    ...
    public String mejorFeo() {
        ...
        for (int i = 1; i < L.size(); i++) {
            Estudiante3A e = L.get(i);
            if (promEst(e.ID) < promEst(best.ID)) { // al revés
                best = e;
            }
        }
    ...
    public String reporteFeisimo() {
        ...
        for (int i = 0; i < L.size(); i++) {
            ...
            r = r + "* " + e.NOMBRE
                    ...
                    + " ING=" + e.CAL[2]
                    + " | prom=" + promEst(e.ID) + "\n";
            if (e.CAL[0] == 0 || e.CAL[1] == 0 || e.CAL[2] == 0) {
                r = r + "  (faltan califs?)\n";
            }
            for (int j = 0; j < 1; j++) {
                r = r + "";
